...
Retrieve a domain controller's DPAPI backup key, optionally specifying a DC and output file:
  ...
  sharp-dpapi backupkey [/nowrap] [/server:SERVER.domain] [/file:key.pvk]


The *search* command will search for potential DPAPI blobs in the registry, files, folders, and base64 blobs:
